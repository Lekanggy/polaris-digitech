...
const FALLBACK_FEATURES = [
  { title: 'Custom Business Email', description: 'Get a professional email address using your company domain powered by Gmail — trusted, secure, and easy to manage.' },
  { title: 'Video Meetings & Conferencing', description: 'Host HD video meetings with up to 150 participants using Google Meet, with recording, noise cancellation, and live captions built in.' },
  { title: 'Cloud Storage', description: 'Store, access, and share files securely from anywhere with Google Drive. Pooled storage shared across your organisation keeps everyone connected.' },
  { title: 'Collaborative Productivity Tools', description: 'Work together in real time on Docs, Sheets, and Slides — no version conflicts, no email attachments, just seamless collaboration.' },
  { title: 'Security & Management Controls', description: 'Centralised admin console gives IT teams full visibility and control over users, devices, and data — with advanced security policies built in.' },
  { title: 'Google Chat & Spaces', description: 'Communicate instantly with direct messages and group spaces. Integrate tasks, files, and meetings directly into your conversations.' },
  { title: 'AppSheet & App Integration', description: 'Build no-code apps on top of your data with AppSheet, and connect Google Workspace to thousands of third-party tools via APIs and Marketplace.' },
  { title: 'Standard Support', description: 'Access 24/7 support from Google specialists, with the option to upgrade to Enhanced Support for faster response times.' },
];
const FALLBACK_PLANS = [
  {
    name: 'Business Starter', price: '$6', period: '/ Month', highlighted: false,
    features: ['Custom and secure business email', '100 participant video meetings', '30 GB pooled storage per user*', 'Security and management controls', 'Standard Support'],
  },
  {
    name: 'Business Standard', price: '$18', period: '/ Month', highlighted: true,
    features: ['Custom and secure business email', '150 participant video meetings + recording', '2 TB pooled storage per user*', 'Security and management controls', 'Standard Support (paid upgrade to Enhanced Support)'],
  },
  {
    name: 'Business Plus', price: '$5', period: '/ Month', highlighted: false,
    features: ['Custom and secure business email + eDiscovery, retention', '500 participant video meetings + recording, attendance tracking', '5 TB pooled storage per user*', 'Enhanced security and management controls, including Vault and advanced endpoint management', 'Standard Support (paid upgrade to Enhanced Support)'],
  },
];
const FALLBACK_INTRO_DESC = 'This is an efficient platform that handles the address verification and management needs of clients with large customer datasets. Our address management portal was designed particularly for the Nigerian address landscape.';
const FALLBACK_KF_TITLE   = 'Key Features of Google Workspace for Business';
const FALLBACK_KF_DESC    = 'Google Workspace for Business brings together communication, collaboration, and productivity tools in a single platform, helping organisations work smarter and more securely.';
const FALLBACK_QUOTE      = 'A one-stop shop tool for everything your business needs in today\'s Digital world.';
const FALLBACK_PRICING_TITLE = 'Choose your Google Workspace pricing plan.';
const FALLBACK_PRICING_DESC  = 'Use flexible, secure tools at no cost with Google Workspace. Google Workspace provides flexible pooled storage per user that is shared across the organisation.';
...
return (
      ...
      <div style={{ maxWidth: '1280px', margin: '0 auto', paddingLeft: 'clamp(24px, 5vw, 80px)', paddingRight: 'clamp(24px, 5vw, 80px)', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1.4fr 1fr', gap: isMobile ? '32px' : 'clamp(40px, 6vw, 80px)', alignItems: 'stretch' }}>
        <motion.div initial={{ opacity: 0, x: -32 }} animate={isVisible ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.7 }} style={{ background: '#E8EAF6', borderRadius: '24px', overflow: 'hidden', width: '100%', aspectRatio: '1 / 1', order: isMobile ? 2 : 1 }}>
          <img src={introImage} alt="Google Workspace for Business" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
        </motion.div>
        <motion.div initial={{ opacity: 0, x: 32 }} animate={isVisible ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.7, delay: 0.1 }} style={{ order: isMobile ? 1 : 2 }}>
          <h1 style={{ fontFamily: satoshi, fontWeight: 700, fontSize: 'clamp(32px, 4.5vw, 56px)', lineHeight: '115%', letterSpacing: '-0.02em', color: '#283172', marginBottom: '24px' }}>Google Workspace for Business</h1>
...
export default function GoogleWorkspaceBusinessPage() {
  ...
  const cms = (data?.partnersProducts ?? []).find(
    p => p.route === 'solutions/google-workspace-business' || p.route === '/solutions/google-workspace-business'
  );

  const introImage  = strapiUrl(cms?.introsection?.leftImage?.url) ?? top4;
  ...
  const quoteImage  = strapiUrl(cms?.quote?.image?.url) ?? gen3;
  ...
  const kfFeatures  = (cms?.features?.features ?? []).length > 0
    ...
    : FALLBACK_FEATURES;
  const showcaseImg = strapiUrl(cms?.showcase?.image?.url) ?? bot4;

  // Pricing — CMS if available, otherwise hardcoded fallback
